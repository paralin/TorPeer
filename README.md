...
p2p discovery & communcation protocol via hidden services on tor.

Concept
=======

Standard p2p discovery requires routers, forwarded ports / NAT hole punching, and reveals client IP addresses. In situations where we'd rather not reveal IP addresses, are operating in a situation without reliable NAT punchthrough / dedicated routing servers, and have a relatively small number of clients, it makes sense to piggyback on the TOR network for peer to peer communication.
...
All communication is done with **proto3**. Over HTTP transports, JSON
